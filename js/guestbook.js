...
(function () {
  ...
  // ── Cooldown timer ─────────────────────────────────────────
  // The window starts silently after a successful post; the visible countdown
  // is only revealed if the user actually tries to post again too soon.
  ...
  // Begin the cooldown window with no visible timer.
  function beginCooldown(seconds) {
    cooldownEndsAt = Date.now() + Math.max(1, Math.round(seconds || COOLDOWN)) * 1000;
  }
  ...
  // Reveal the countdown (button label + draining bar) for the time left.
  function revealTimer() {
    if (timerVisible || !inCooldown()) return;
    timerVisible = true;
    ...
    cooldownTimer = setInterval(tickTimer, 250);
  }
  ...
  form.addEventListener('submit', async (ev) => {
    ...
    if (inCooldown()) {
      revealTimer(); // user tried again too soon → now show the countdown
    ...
    try {
      ...
      if (r.ok && data.entry) {
        ...
        revealTimer(); // server says too soon → show the countdown
      ...
      // After a first post the button returns to normal (timer stays hidden);
      // only leave it alone when the countdown is actually being shown.
      if (!timerVisible) {
        submitBtn.disabled = false;
        if (submitLbl) submitLbl.textContent = 'Sign Guestbook';
      }
